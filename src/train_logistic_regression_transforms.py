...
class LogisticRegression(torch.nn.Module):
    def __init__(self, num_components, pca, random_crop_size=None, compute_scattering=False, J=2, global_pca = True):
        super(LogisticRegression, self).__init__()            
        self.compute_scattering = compute_scattering
        self.J = J
...
def define_model(device, params, num_components, pca, compute_scattering = False, J=2, random_crop_size = None,
                global_pca = True):
    
    model_ft = LogisticRegression(num_components = num_components, pca = pca,
                                    compute_scattering = compute_scattering , J=J ,
...
def get_transformed_images(dataloader,  n_components, device, J, seed):
    ...
    if J is not None:
        logging.info(f'Scattering Wavelet transforms started.')
        use_cuda = torch.cuda.is_available()
        S = Scattering2D(J,(images.shape[2], images.shape[3]))
        if use_cuda: S = S.cuda()
        images = S.scattering(torch.from_numpy(images).to(device))
        logging.info(f'Scattering Wavelet transforms done.')
...
def train_predict(catalog, params):
    ...
    is_raw_data = params['model']['is_raw_data']
    is_transform = params['model']['transform']
    compute_scattering = False
    ...
    if is_raw_data:
        ...
        params['scattering']['J'] = J = None
        params['scattering']['max_order'] = None
    
    elif is_raw_data is False and is_transform:
    # in the case where we want to use the scattering with transformations,
    # we need to dowload the raw images, then apply random transformations
    # extract sacttering coefficients and finally compute pca 
        data = reshape_raw_images(df, params['preprocess']['dimension']['M'], params['preprocess']['dimension']['N'] )
        J = params['scattering']['J']
        compute_scattering  = True # we will need to compute the scattering during the training of the model
    else:
        J = params['scattering']['J']
    ...
    for train_index, test_index in group_kfold_test.split(df, df_y, df_pid):
        ...
        # Raw images 
        if is_raw_data or (is_raw_data is False and is_transform):
        ...
        # If there is no transformations
        if params['model']['transform']:
            subtrain_data_flatten = get_transformed_images(dataloaders['train'], 
                                                            params['pca']['n_components'], 
                                                            device, J, seed)
        ...
        model, criterion, optimizer, scheduler = define_model(device, params['model'], 
                                                                ...
                                                                pca= pca, compute_scattering = compute_scattering,
                                                                J = J,
