...
class LogisticRegression(torch.nn.Module):
    def __init__(self, num_components, pca, J=3):
        super(LogisticRegression, self).__init__()      
        # local PCA - using convolution of filter 1 x1      
        self.pca_layer = nn.Conv2d(81, num_components, 1)
        ...
        if J==2:
            self.linear = torch.nn.Linear(num_components * 108 * 159, 1)

        elif J==3:
            self.linear = torch.nn.Linear(num_components * 54 * 79, 1)

        elif J==4:
            self.linear = torch.nn.Linear(num_components * 27 * 39, 1)
        
        elif J==5:
            self.linear = torch.nn.Linear(num_components * 13 * 19, 1)
        
        elif J==6:
            self.linear = torch.nn.Linear(num_components * 6 * 9, 1)
        else:
    ...
    def forward(self, x):
        with torch.no_grad():
            if self.J  == 2:
                pca_components = self.pca_layer(x.view(x.shape[0], 81, 108, 159))
            elif self.J==3:
                pca_components = self.pca_layer(x.view(x.shape[0], 217, 54, 79))
            elif self.J==4:
                pca_components = self.pca_layer(x.view(x.shape[0], 417, 27, 39))
            elif self.J==5:
                pca_components = self.pca_layer(x.view(x.shape[0], 681, 13, 19))
            elif self.J==6:
                pca_components = self.pca_layer(x.view(x.shape[0], 1009, 6, 9))
            else:
                raise NotImplemented(f"J {self.J} parameter for scattering not implemented")
        pca_components_flatten = pca_components.view(pca_components.shape[0],-1)
        y_pred = F.sigmoid(self.linear(pca_components_flatten))
...
def define_model(device, params, num_components, pca, J):
    model_ft = LogisticRegression(num_components= num_components, pca = pca, J=J)
