...
def get_metrics(labels, preds, logits):
    '''
...
def get_majority_vote(y_test, predictions):
    ...
    for i in range(0,len(predictions),  num_img_per_pat):
        ...
        counter_pred = Counter(np.array(predictions)[idx])
        majority_vote_predictions.append(int(counter_pred.most_common(1)[0][0]))
        majority_vote_labels.append(int(np.array(y_test)[i]))
    return get_metrics(majority_vote_labels, majority_vote_predictions, majority_vote_predictions)
...
def log_test_metrics(test_metrics, test_metrics_mv,params):
    '''
    ...
    logging.info(f'Test Average: acc: {np.nanmean(test_acc)} , auc: {np.nanmean(test_auc)}, \
                                        specificity: {np.nanmean(test_specificity)}, sensitivity: {np.nanmean(test_sensitivity)}')
    logging.info(f'Test Average Majority Vote: acc: {np.nanmean(test_acc_mv)} , auc: {np.nanmean(test_auc_mv)},\
                                        specificity: {np.nanmean(test_specificity_mv)}, sensitivity: {np.nanmean(test_sensitivity_mv)}')
